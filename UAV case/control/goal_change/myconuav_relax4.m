function [c,ceq] = myconuav_relax4(x)
...
c = [c, - info_now + (configure.forensic_target - y(end-2)*x(end-(bound_o + bound_p + 3)-2))];
c = [c,time_now - (configure.Time_target + y(end-1)*x(end-(bound_o + bound_p + 3)-1))];
c = [c, energy_now - (configure.battery_target + y(end)*x(end-(bound_o + bound_p + 3)))];
...
for j = 1: length_o
    for i = 1:initial_N + 1
        ...
        c = [c, - dis_o(i, j) + (configure.radius + configure.obstacle_radius + configure.obstacle_max - y((j-1) * (initial_N+1) + i)*x(x_index))];
    end
end
...
for j = 1: length_p
    for i = 1:initial_N + 1
        ...
        c = [c, - dis_p(i, j) + (configure.radius + configure.privacy_radius + configure.privacy_max - y(bound_o +(j-1) * (initial_N+1) + i)*x(x_index))];
    end
end
