...
Pre = [data1_uav',data2_uav',data3_uav'];
g1 = [ones(size(data1_uav)); 2*ones(size(data2_uav)); 3*ones(size(data3_uav));];
g1 = g1(:);
% g2 = repmat(1:3,8977*6,1); 
g2 = [ones(size(data1_uav)); 2*ones(size(data2_uav)); 3*ones(size(data3_uav));];
g2 = g2(:);
...
x=0.7:0.1:1.3;
y=[0.2039,0.2039,0.2039,0.2039,0.2039,0.2039,0.2039];
plot(x,y, ':', 'Color',[0.6350 0.0780 0.1840],'linewidth',2)
x=1.85:0.1:2.45;
y=[0.0991,0.0991,0.0991,0.0991,0.0991,0.0991,0.0991];
plot(x,y, ':','Color',[0.6350 0.0780 0.1840],'linewidth',2)
x=3.02:0.1:3.62;
y=[0.1301,0.1301,0.1301,0.1301,0.1301,0.1301,0.1301];
plot(x,y,':','Color',[0.6350 0.0780 0.1840],'linewidth',2)
