function [data, trajectory,velocity_history,planning_time, flag] = uav_initial(num)
global env
global env_known
global configure
configure = Configure();
...
while (1)
    needplan = 0;
    fprintf(2,'uav_initial: current step %d\n', current_step);
    t1=clock;
    if current_point(1) == end_point(1) && current_point(2) == end_point(2) && current_point(3) == end_point(3)
        ...
        [SR, PR] = caculate_risk(trajectory, env);
        data = [DS_i, DS_t, DS_e, SR, PR, plan_num];
...
%         xlswrite('data.xls',data,sheet,xlsRange);
    ...
    if size(following_plan, 1) == 1
        ...
        time = time + dis/sqrt(following_plan(1,1)^2 + following_plan(1,2)^2 + following_plan(1,3)^2);
...
%         lb,ub
%         mycon2(x0)
        [x,fval,exitflag]=fmincon(@obj2,x0,[],[],[],[],lb,ub,@mycon2, options);
