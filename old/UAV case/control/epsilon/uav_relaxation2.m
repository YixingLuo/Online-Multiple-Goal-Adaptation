...
% clear
% num = 1;
function [data, trajectory,velocity_history,planning_time, rate_list, tag_list] = uav_relaxation2(num)
global env
global env_known
global configure
global eplison
...
%         save(name3, 'velocity_history');
    ...
    if current_step > configure.Time_budget/configure.Time_step + 1
...
%         options.MaxFunEvals = 100000;
        [x,fval,exitflag]=fmincon(@objuav_relax4,x0,[],[],[],[],lb,ub,@myconuav_relax4,options);
       
        tau = configure.Time_step;

        iternum = iternum + 1;
        [safety_ratio,  privacy_ratio, info_ratio, time_ratio, energy_ratio] = goal_selection2(x);
...
%        [safety_variance, safety_ratio, privacy_variance, privacy_ratio, info_variance, info_ratio, time_variance, time_ratio, energy_variance, energy_ratio] = goal_selection2(x);
%        ratio = [safety_ratio, privacy_ratio, info_ratio, time_ratio, energy_ratio];
       exitflag_relax = 0;
       iternum_relax = 0;
...
%               end  
       ...
       if iternum_relax > 20 && exitflag_relax <=0
