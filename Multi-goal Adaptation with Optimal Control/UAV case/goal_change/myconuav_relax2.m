function [c,ceq] = myconuav_relax2(x)
c=[];
ceq=[];
global configure
...
for j = 1: length_o
    for i = 1:initial_N + 1
        c = [c, - dis_o(i, j) + (configure.radius + configure.obstacle_radius + x(end-4))];
    end
end

for j = 1: length_p
    for i = 1:initial_N + 1
        c = [c, - dis_p(i, j) + (configure.radius + configure.privacy_radius + x(end-3))];
    end
end
