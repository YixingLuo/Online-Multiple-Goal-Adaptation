...
Pre = [data1_uuv',data2_uuv',data3_uuv',data1_uav',data2_uav',data3_uav'];
g1 = [ones(size(data1_uuv)); 2*ones(size(data2_uuv)); 3*ones(size(data3_uuv)); 4*ones(size(data1_uav)); 5*ones(size(data2_uav));...
    6*ones(size(data3_uav));];
g1 = g1(:);
% g2 = repmat(1:3,8977*6,1); 
g2 = [ones(size(data1_uuv)); 2*ones(size(data2_uuv)); 3*ones(size(data3_uuv)); 4*ones(size(data1_uav)); 5*ones(size(data2_uav));...
    6*ones(size(data3_uav));];
g2 = g2(:);
...
ylabel('Overhead [s]','Fontname', 'Times New Roman','FontSize',16);
grid on
% set(gca,'YLim',[0,0.8]);
...
plot(5.2,0.2039,'Color',[0.6350 0.0780 0.1840],'Marker', '*')
plot(6.6,0.0991,'Color',[0.6350 0.0780 0.1840],'Marker', '*')
plot(8,0.1301,'Color',[0.6350 0.0780 0.1840],'Marker', '*')
hLegend = legend(findall(gca,'Tag','Box'), {'Captain','GSlack','AMOCS-MA'},'Box','off','FontSize',12,'Location','NorthWest');
h = findobj(gca,'Tag','Median');
set(h,'Visible','on');
set(gca,'FontSize',14)
